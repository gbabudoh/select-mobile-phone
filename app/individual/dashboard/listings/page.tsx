...
export default function IndividualListingsPage() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (remainingSlots <= 0) {
      showToast("Maximum 4 images allowed per device listing.");
      return;
    }
  ...
  const handleCreateListing = async (e: React.FormEvent) => {
    ...
    const newListing: DeviceListing = {
    ...
    try {
      if (session?.user) {
        await fetch("/api/listings", {
          ...
          body: JSON.stringify({
            sellerId: (session.user as { id?: string }).id,
            title,
            brand,
            category,
            price: Number(price),
    ...
    setSubmitting(false);
    handleCloseModal();
    showToast(`Listing "${title}" posted successfully!`);
  };

  const handleDeleteListing = (id: string, itemTitle: string) => {
    setListings((prev) => prev.filter((l) => l.id !== id));
    showToast(`Removed "${itemTitle}" from your listings.`);
  };
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-[#0f172a] tracking-tight">My Device Listings</h1>
          <p className="text-sm text-slate-500 font-medium mt-1">
            Manage your personal device inventory, active offers, and trade-in sales.
          </p>
        </div>
        <button
      ...
      <div className="p-4 rounded-3xl bg-white border border-slate-200/80 shadow-sm flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative flex-1 w-full">
          ...
          <input
            ...
            placeholder="Search listings by model, brand, or condition..."
      ...
      {filteredListings.length > 0 ? (
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-200/80 p-16 text-center shadow-sm space-y-4">
          ...
          <p className="text-xs text-slate-500 font-medium max-w-md mx-auto">
            List your phone, tablet, or accessory to reach thousands of verified buyers across the US &amp; Canada instantly.
          </p>
          <button
            onClick={handleOpenModal}
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-black text-[#0f172a]">List New Device</h3>
                    <p className="text-xs text-slate-500 font-medium">Create a verified listing for the marketplace</p>
                  </div>
                </div>
              ...
              <form onSubmit={handleCreateListing} className="space-y-5">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1.5">Device Title</label>
                  <input
                    type="text"
                    required
                    value={title}
                    onChange={(e) => setTitle(e.target.value)}
                    placeholder="e.g. iPhone 16 Pro 256GB - Natural Titanium"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1.5">Selling Price ($)</label>
                    <input
                      type="number"
                      required
                      min={1}
                  ...
                  <div>
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1.5">Shipping Region</label>
                    <select
                      value={country}
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1.5">IMEI Number (Optional)</label>
                  <input
                    type="text"
                    maxLength={15}
                    value={imei}
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                      <Camera className="w-3.5 h-3.5 text-[#04a1c6]" /> Device Photos (Maximum 4 Images)
                    </label>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    {uploadedImages.map((imgSrc, idx) => (
                      <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden border border-slate-200 group bg-slate-50">
                        <Image src={imgSrc} alt={`Device upload ${idx + 1}`} fill className="object-cover" />
                        <button
                          type="button"
                ...
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 block mb-1.5">Description &amp; Notes</label>
                  <textarea
                    rows={3}
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder="Provide additional details about battery health, original box, charger..."
                    className="w-full px-4 py-3 rounded-2xl border border-slate-200 text-xs font-bold text-slate-900 focus:outline-none"
                  />
                </div>
                ...
                >
                  {submitting ? "Publishing Listing..." : "Post Device Listing"}
                </button>
              </form>
            </motion.div>
          </motion.div>
